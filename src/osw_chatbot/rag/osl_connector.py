...
def get_full_context(subject=None):
    from SPARQLWrapper import JSON, SPARQLWrapper  # pip install sparqlwrapper

    sparql_url = "https://graph.battery.knowledge-graph.eu/blazegraph/namespace/kb/sparql"  # noqa
    sparql_query = """
        ...
        try:
            if first_run:
                ...
                for ns in nss:
                    ...
                    while _continue:
                        driver.get(
                            server
                            + "/w/api.php?action=query&format=json&prop=revisions&generator=allpages&formatversion=2&rvprop=timestamp&gaplimit=10&gapnamespace="  # noqa
                            + ns
                            + continue_param
                        )
                        ...
                        if "continue" in parsed_json:
                            print(
                                "Continue with page "
                                + parsed_json["continue"]["gapcontinue"]
                            )
                            continue_param = (
                                "&continue="
                                + parsed_json["continue"]["continue"]
                                + "&gapcontinue="
                                + parsed_json["continue"]["gapcontinue"]
                            )
                        else:
                ...
                timestamp = (
                    (datetime.today() - timedelta(hours=0, minutes=10))
                    .replace(microsecond=0)
                    .isoformat()
                )
                driver.get(
                    server
                    + "/w/api.php?action=query&list=recentchanges&rcprop=title|ids|sizes|flags|user&rclimit=100&rcend="  # noqa
                    + timestamp
                )
                content = driver.find_element(By.TAG_NAME, "pre").text
                parsed_json = json.loads(content)
            ...
            for page in pages:
                ...
                if mode == "html":
                    ...
                    html = driver.find_elements(By.ID, "mw-content-text")[
                        0
                    ].get_attribute("innerHTML")
                    with open(path + page_name + ".html", "w") as f:
                        f.write(html)
                    # screenshot = driver.save_screenshot(path + page_name + '.png') # noqa
                if mode == "sparql":
                    page_name = page.replace(":", "_")
                    print("SPARQL " + page)
                    ...
                    print(context_text)
                    try:
                        with open(path + page_name + ".txt", "w") as f:
            ...
            # wiki_url = driver.find_element(By.CLASS_NAME, 'iUh30').text.replace(' › ','/') # noqa
            # driver.get(wiki_url)#
