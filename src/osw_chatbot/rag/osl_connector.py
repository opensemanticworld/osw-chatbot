            ...
            if first_run:
                ...
                for ns in nss:
                    ...
                    while _continue:
                        driver.get(
                            server
                            + "/w/api.php?action=query&format=json&prop=revisions&generator=allpages&formatversion=2&rvprop=timestamp&gaplimit=10&gapnamespace="
                            + ns
                            + continue_param
                        )
                ...
                driver.get(
                    server
                    + "/w/api.php?action=query&list=recentchanges&rcprop=title|ids|sizes|flags|user&rclimit=100&rcend="
                    + timestamp
                )
                content = driver.find_element(By.TAG_NAME, "pre").text
                parsed_json = json.loads(content)
