...
class WebLink(BaseModel):
    url: str
    """The URL of the search result. This link can be followed to learn more."""  # noqa
    title: str
...
async def web_search_tool(param: WebSearchParam) -> WebSearchResult:
    """
    Uses a search engine to search the web for the
    given query and returns the results.
    """
...
async def web_search(param: WebSearchParam) -> WebSearchResult:
    """
    Uses a search engine to search the web
    for the given query and returns the results.
    """

    # GET https://www.startpage.com/sp/search?query=<query>&cat=web&pl=opensearch&language=english # noqa
    url = f"https://www.startpage.com/sp/search?query={param.query}&cat=web&pl=opensearch&language=english"  # noqa
    # url = "https://www.startpage.com/sp/search?query=MP%C2%B3ROVE+sinterpasten&cat=web&pl=opensearch&language=english" # noqa
    # headers = {
    ...
    # get the title from the text value of subelements with css class "result-title" # noqa
    # get the url from the text value of subelements with css class "result-link" # noqa
    # get the description from the text value of subelements with css class "description" # noqa
    ...
    #    results.append(SearchResult(url=url, title=title, description=description)) # noqa

    # select text value per regex: <p class="description css-1507v2l">mAgic- und magiCu-<b>Sinterpasten</b> für Hochleistungsanwendungen. Höhere Leistungsdichten gehen mit höheren Betriebstemperaturen einher. Gleichzeitig müssen Geräte&nbsp;...</p> # noqa
    ...
    async with async_playwright() as playwright:
        ...
        if headless:
            user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:128.0) Gecko/20100101 Firefox/128.0"  # noqa
        ...
        for element in await page.query_selector_all(".result"):
            try:
                title = await (
                    await element.query_selector(".result-title")
                ).text_content()
                url = await (
                    await element.query_selector(".result-link")
                ).get_attribute("href")
                description = await (
                    await element.query_selector(".description")
                ).text_content()
                results.append(
...
class BrowseWebPageParam(BaseModel):
    ...
    """The zero-based index of the current requested content chunk if the page content is to large to fit in a single context."""  # noqa
...
class BrowseWebPageResult(BaseModel):
    ...
    # rag_query: str = ""
    # """If the page content is to large for single context, the RAG query to be used to generate a reduced text and link list.""" # noqa
    chunk: Optional[int] = 0
    """The zero-based index of the current content junk the page content is to large. If < total_chunks -1 request the next chunk."""  # noqa
...
async def browse_web_page(param: BrowseWebPageParam) -> BrowseWebPageResult:
    ...
    # if if total token length of text is larger than 4096, reduce the text and links # noqa
    ...
    if chunks > 1:
        if param.chunk < text_chunk_count:
            result = BrowseWebPageResult(
                text=text_chunks[param.chunk],
                links=[],
                chunk=param.chunk,
                total_chunks=chunks,
            )
            ...
            result = BrowseWebPageResult(
                text="",
                links=link_chunks[param.chunk - text_chunk_count],
                chunk=param.chunk,
                total_chunks=chunks,
            )
        # result = BrowseWebPageResult(text="", links=link_chunks[param.chunk], chunk=param.chunk, total_chunks=link_chunk_count) # noqa
...
prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            str(
                (
                    "You are a helpful assistant. Try to answer the user's question directly. If you need additional information, search the web for it. ",  # noqa
                    "If a result page contains helpful links you can follow them if needed but try to minimize the number of links you follow.",  # noqa
                    "If the user explicitly provides a link you must follow it to get the information.",  # noqa
                    "Provide the user with a summary of the information you found and, if applicable, links to the most relevant pages.",  # noqa
                )
            ),
        ),
